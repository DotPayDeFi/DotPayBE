...
if [[ "${PROVIDER}" == "cloudflared" ]]; then
  ...
  echo "Unsupported TUNNEL_PROVIDER=${PROVIDER}. Use cloudflared or localhostrun."
  exit 1
fi
...
for _ in $(seq 1 300); do
  ...
  if [[ "${PROVIDER}" == "cloudflared" ]]; then
    ...
    URL="$(grep -Eo 'https://[A-Za-z0-9.-]+\.trycloudflare\.com' "${LOG_FILE}" 2>/dev/null | tail -n 1 | tr -d '\r' || true)"
  else
    # Pick the last public URL announced by localhost.run.
    URL="$(awk '/tunneled with tls termination/ {print $NF}' "${LOG_FILE}" 2>/dev/null | tail -n 1 | tr -d '\r' || true)"
  fi
  if [[ -n "${URL}" ]]; then
    break
  fi
