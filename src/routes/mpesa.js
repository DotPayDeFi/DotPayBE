...
function extractPhone(body) {
  const msisdn = normalizeMsisdn(body?.phone || body?.phoneNumber);
  return msisdn;
}
...
function parseStkMetadata(stkCallback) {
  ...
  return {
    ...
    amount: byName.get("Amount") || null,
    phoneNumber: byName.get("PhoneNumber") ? String(byName.get("PhoneNumber")) : null,
    transactionDate: byName.get("TransactionDate") || null,
  };
}
...
router.post("/deposit", requireClientAuth, enforceRateLimit, async (req, res) =>
  handleC2BStk({
    req,
    res,
    product: "deposit",
...
router.post("/buy-crypto", requireClientAuth, enforceRateLimit, async (req, res) =>
  handleC2BStk({
    req,
    res,
    product: "buy_crypto",
...
router.post("/pay/paybill", requireClientAuth, enforceRateLimit, async (req, res) => {
...
router.post("/pay/till", requireClientAuth, enforceRateLimit, async (req, res) =>
  handleB2B({
    req,
    res,
    product: "till",
...
router.post("/pay-with-crypto", requireClientAuth, enforceRateLimit, async (req, res) => {
...
router.post("/withdraw", requireClientAuth, enforceRateLimit, async (req, res) =>
  handleB2C({ req, res, product: "withdraw" })
);

router.post("/crypto-to-mpesa", requireClientAuth, enforceRateLimit, async (req, res) =>
  handleB2C({ req, res, product: "crypto_to_mpesa" })
);

router.post("/submit-receipt", requireClientAuth, async (req, res) => {
...
router.get("/transaction/:transactionId", requireClientAuth, async (req, res) => {
...
router.post("/stk-callback", async (req, res) => {
  if (!verifyCallbackToken(req)) {
    return fail(res, 401, "Unauthorized callback token");
  }
...
router.post("/b2b-callback", async (req, res) => {
  if (!verifyCallbackToken(req)) {
    return fail(res, 401, "Unauthorized callback token");
  }
...
router.post("/queue-timeout", async (req, res) => {
  if (!verifyCallbackToken(req)) {
    return fail(res, 401, "Unauthorized callback token");
  }
